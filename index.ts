...
const atmMachine = async (users: User[]) => {
  ...
  if (user) {
    console.log(
      chalk.green.underline(`Welcome ${chalk.bold.blueBright(user.name)}!`)
    );
    console.log(
      `Account Number: ${chalk.bold.underline.blue(user.accountNumber)}`
    );
    ...
    case "withdraw":
      ...
      switch (price.amount) {
        ...
        case "Other":
          ...
          console.log(
            chalk.green(`${customAmount.amount} withdrawn successfully!`)
          );
          console.log(`Balance Amount: ${chalk.bold.blue(user.balance)}`);
          break;
        default:
